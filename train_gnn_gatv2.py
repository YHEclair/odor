...
def predict_probs(model, loader, device, desc="Predicting"):
    ...
    for batch_data in pbar:
        ...
        probs = torch.sigmoid(logits).cpu().numpy()
        y_true = batch_data.y.view(batch_data.num_graphs, -1).cpu().numpy()
...
def main():
    ...
    logger.info(f"Auto-detected edge feature dim: {sample_edge_attr.shape[1] if sample_edge_attr.numel() > 0 else 16}")
    ...
    valid_probs, valid_targets = predict_probs(
        ...
        loader=valid_loader,
        device=cfg.device,
        desc="Export Valid Probs"
    )

    test_probs, test_targets = predict_probs(
        model=model,
        loader=test_loader,
        device=cfg.device,
        desc="Export Test Probs"
    )

    np.save(os.path.join(cfg.save_dir, "gnn_valid_probs.npy"), valid_probs)
    np.save(os.path.join(cfg.save_dir, "gnn_valid_targets.npy"), valid_targets)
